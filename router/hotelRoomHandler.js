...
router.get("/getById/:id", async (req, res) => {
  ...
  const hotelSeats = await HotelRooms.find({ tour_id: id }).select({
    name: 1,
    seat_no: 1,
  });
...
router.post("/getUniqueRoomNumbers", async (req, res) => {
  ...
  const hotelSeats = await HotelRooms.find({
    name: name,
    tour_id: tourId,
  }).distinct("roomId");
  res.send({ data: hotelSeats });
...
router.post("/getBookingListByTourId", async (req, res) => {
  const tourId = req.body.tourId;
  const bookingIdList = await Booking.find({
    tourId: tourId,
    payment: "yes",
  }).distinct("_id");
...
router.post("/getUniqueSeatNumberByHotelId", async (req, res) => {
  ...
  const personListInfo = await BookingPerson.find({
...
router.get("/getHotelInformationByName/:hotelName", async (req, res) => {
  ...
  const hotelInformation = await HotelRooms.find({ name: name }).select({
    name: 1,
    roomId: 1,
  });
...
router.post("/assignHotelForTour", async (req, res) => {
  ...
  let doc = await HotelRooms.updateMany(
    { hotel_id: req.body.hotelId },
    { $set: { tour_id: req.body.tourId } },
    { multi: true }
  );
  if (doc.acknowledged === true) {
    res.send({
      message: `Booking Hotel Successfull for the tour ${req.hotelId}`,
    });
  } else {
...
router.get("/mail-send", async (req, res) => {
  const sendEmail = await sendMail(
    NotFillApplicationSubject,
    NotFillApplicationBody,
    "sahaanik1048@gmail.com"
  );
  if (!sendEmail) {
